            ...
            Ok(msg) => {
                ...
                match msg.update_oneof {
                    ...
                    Some(UpdateOneof::Transaction(msg)) => {
                        ...
                        let signature_str = Signature::try_from(tx.signature.as_slice())
                            .context("invalid signature")?
                            .to_string();
                        let uid = Some(format!("transaction.{signature_str}"));

                        let mut value = create_pretty_transaction(tx)?;
                        value["slot"] = json!(msg.slot);

                        stream_update("transaction", created_at, &filters, value, uid);
                    }
                    Some(UpdateOneof::TransactionStatus(msg)) => {
                        let uid = Some(format!(
                            "transactionStatus.{}.{}",
                            Signature::try_from(msg.signature.as_slice())
                                .context("invalid signature")?
                                .to_string(),
                            msg.index,
                        ));
                    ...
                    Some(UpdateOneof::Entry(msg)) => {
                        let uid = Some(format!("entry.{}.{}", msg.slot, msg.index));
                        stream_update("entry", created_at, &filters, create_pretty_entry(msg)?, uid);
                    }
                    Some(UpdateOneof::BlockMeta(msg)) => {
                        stream_update(
                            ...
                            Some(format!("blockmeta.{}", msg.slot)),
                        );
                    }
...
fn create_pretty_transaction(tx: SubscribeUpdateTransactionInfo) -> anyhow::Result<Value> {
    Ok(json!({
        ...
        "is_vote": tx.is_vote,
        "index": tx.index,
        "transaction": convert_from::create_tx_with_meta(tx)
            .map_err(|error| anyhow::anyhow!(error))
            .context("invalid tx with meta")?
            .encode(UiTransactionEncoding::Json, Some(u8::MAX), true)
...
fn stream_update(kind: &str, created_at: SystemTime, filters: &[String], value: Value, uid: Option<String>) {
    let unix_since = created_at
        .duration_since(UNIX_EPOCH)
        .expect("valid system time");
    
    let output = json!({
        "source": "trading.geyser.yellowstone",
        "kind": kind,
        "filters": filters,
        "timestamp_us": unix_since.as_secs() * 1_000_000 + unix_since.subsec_micros() as u64,
        "data": value,
        "uid": uid,
    });
    
    info!("{}", serde_json::to_string(&output).expect("json serialization failed"));
}
